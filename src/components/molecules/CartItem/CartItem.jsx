...
export function CartItem({ item, onUpdateQuantity, onDelete, onFavorite }) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center py-6 border-b border-[#F5F0E6] gap-6 last:border-0 relative">
      <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-xl flex-shrink-0 relative overflow-hidden ${item.bgColor || 'bg-[#F2EDEA]'}`}>
      ...
      <div className="flex-1 flex flex-col pt-1">
        ...
        <div className="flex items-center justify-between mt-auto w-full">
          ...
          <div className="flex flex-col md:absolute md:right-0 md:top-6 items-end gap-2 text-right">
             <button onClick={() => onFavorite(item.id)} className="text-sm text-[#6B4F3B] hover:text-[#D47112] font-sans">favorite</button>
             <button onClick={() => onDelete(item.id)} className="text-sm text-[#6B4F3B] hover:text-[#C15C5C] font-sans">delete</button>
          </div>
