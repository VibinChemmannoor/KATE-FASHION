...
export function WishlistItem({ item, onRemove, onAddToCart }) {
  return (
    <div className="group relative flex flex-col">
      {/* Image Container */}
      <div className={`relative w-full aspect-[4/5] rounded-xl overflow-hidden mb-4 ${item.bgColor || 'bg-[#FDFBF7]'}`}>
        <div className="absolute inset-0 flex items-center justify-center">
           <span className="text-[#6B4F3B]/40 font-serif italic text-sm text-center px-4">{item.imageAlt || 'Product Image'}</span>
        </div>
        ...
        {/* Badges */}
        {item.badge && (
          <span className={`absolute top-3 left-3 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${item.badge === 'SALE' ? 'bg-[#EC7F13]' : 'bg-[#412B1F]'}`}>
            {item.badge}
          </span>
        )}
      </div>
      ...
      <div className="flex justify-between items-start mb-1">
        ...
        <div className="flex flex-col items-end">
          <span className={`text-sm font-bold ${item.salePrice ? 'text-[#EC7F13]' : 'text-[#4A3525]'}`}>
            {item.salePrice || item.price}
          </span>
          {item.salePrice && (
            <span className="text-xs text-[#6B4F3B]/50 line-through">
              {item.price}
            </span>
          )}
        </div>
