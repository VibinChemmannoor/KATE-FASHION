...
export function AddressSelector({ addresses, onAddNew, onProceedToPayment }) {
  ...
  return (
    <div className="w-full">
      <h2 className="text-3xl font-bold font-serif text-[#1a1b26] mb-2 tracking-tight">Select Shipping Address</h2>
      <p className="text-sm font-sans text-[#6B4F3B]/70 mb-8">Choose your delivery destination or add a new one.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          ...
          return (
            <div 
              ...
              className={`relative border-2 rounded-xl p-6 transition-all ${
                isSelected ? 'border-[#EF831D] bg-white' : 'border-[#EAE4DD] bg-[#FAF9F6] hover:border-[#D47112]/50'
              }`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold font-sans text-[#4A3525] leading-tight">{addr.fullName}</h3>
                    <p className="text-xs font-bold text-[#6B4F3B]/60 uppercase tracking-widest mt-1">Home Address</p>
                  </div>
                </div>
                {addr.isDefault && (
        ...
        >
          ...
          <div>
            <span className="block text-sm font-bold font-sans text-[#4A3525] mb-1">Add New Address</span>
            <span className="text-xs font-sans text-[#6B4F3B]/60">For gifts or secondary locations</span>
          </div>
        </button>
      </div>
